...
fn ArgFind(args: [][*:0]const u8, name: []const u8) ?[]const u8 {
    for (args) |arg| {
        const len = std.mem.len(arg);
        if (len > 4 and
            arg[0] == '-' and
            arg[1] == '-' and
            std.mem.startsWith(u8, arg[2..len], name))
        {
            if (arg[2 + name.len] == '=') {
                return arg[2 + name.len + 1 .. len];
            }
        }
    }
    return null;
...
fn ArgFloat(args: [][*:0]u8, name: []const u8, defaultValue: f32) !f32 {
    if (ArgFind(args, name)) |value| {
...
fn ArgInt(args: [][*:0]u8, name: []const u8, defaultValue: i32) !i32 {
    if (ArgFind(args, name)) |value| {
    ...
    //
    // errno = 0;
    // const output = strtol(value, NULL, 10);
    // if (errno == 0) { printf("INFO: Parsed option '%s' as '%s'\n", name, value); return output; }
    //
...
fn ArgBool(args: [][*:0]u8, name: []const u8, defaultValue: bool) !bool {
    if (ArgFind(args, name)) |value| {
...
fn ArgVector3(args: [][*:0]u8, name: []const u8, defaultValue: raylib.Vector3) !raylib.Vector3 {
    if (ArgFind(args, name)) |value| {
        _ = value;
        // var cx: f32 = 0;
        // var cy: f32 = 0;
        // var cz: f32 = 0;
        // if (std.c.sscanf(value, "%f,%f,%f", &cx, &cy, &cz) == 3) {
        //     // printf("INFO: Parsed option '%s' as '%s'\n", name, value);
...
const OrbitCamera = struct {
    ...
    pub fn new(args: [][*:0]u8) !OrbitCamera {
        return .{
            .cam3d = .{
                .position = .{ .x = 2.0, .y = 3.0, .z = 5.0 },
                .target = .{ .x = -0.5, .y = 1.0, .z = 0.0 },
                .up = .{ .x = 0.0, .y = 1.0, .z = 0.0 },
                .fovy = try ArgFloat(args, "cameraFOV", 45.0),
                .projection = raylib.CAMERA_PERSPECTIVE,
            },
            .azimuth = try ArgFloat(args, "cameraAzimuth", 0.0),
            .altitude = try ArgFloat(args, "cameraAltitude", 0.4),
            .distance = try ArgFloat(args, "cameraDistance", 4.0),
            .offset = try ArgVector3(args, "cameraOffset", .{ .x = 0, .y = 0, .z = 0 }),
            .track = try ArgBool(args, "cameraTrack", true),
            .trackBone = try ArgInt(args, "cameraTrackBone", 0),
        };
    }
};
...
fn ShaderUniformsInit(uniforms: *ShaderUniforms, shader: raylib.Shader) void {
    _ = uniforms;
    _ = shader;
...
pub fn main() !void {
    ...
    const screenWidth = try ArgInt(std.os.argv, "screenWidth", 1280);
    const screenHeight = try ArgInt(std.os.argv, "screenHeight", 720);

    // Init Window

    raylib.SetConfigFlags(raylib.FLAG_VSYNC_HINT);
    raylib.SetConfigFlags(raylib.FLAG_MSAA_4X_HINT);
    raylib.InitWindow(screenWidth, screenHeight, "BVHView");
    raylib.SetTargetFPS(60);
    const shader = raylib.LoadShaderFromMemory(shaderVS, shaderFS);
    ...
    const app = ApplicationState{
        ...
        // Camera
        .camera = try OrbitCamera.new(std.os.argv),
        .shader = shader,
        .uniforms = ShaderUniformsInit(shader),
    };
