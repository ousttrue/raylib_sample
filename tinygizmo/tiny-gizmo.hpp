...
struct gizmo_application_state {
  ...
  minalg::float3
      ray_origin; // world-space ray origin (i.e. the camera position)
  minalg::float3 ray_direction; // world-space ray direction
  camera_parameters cam; // Used for constructing inverse view projection for
                         // raycasting onto gizmo geometry
};
...
struct gizmo_context {
  ...
  std::span<gizmo_renderable> drawlist();
  transform_mode get_mode()
      const; // Return the active mode being used by `transform_gizmo(...)`
};

bool transform_gizmo(const std::string &name, gizmo_context &g,
                     rigid_transform &t);
